...
%\renewcommand\thesection{\arabic{section}} % Makes sections x istead of x.x
\begin{document}

\RedeclareSectionCommand[
  beforeskip=0ex, % Abstand vor \chapter
  afterskip=0.5ex plus .2ex              % Abstand nach \chapter
]{chapter}
\frontmatter
\pagenumbering{Roman} 				% needed to capitalize the roman page numbering
\input{frontmatter/1_title}	
\tableofcontents

\mainmatter
\input{mainmatter/1_Stakeholder_Interviews}
\input{mainmatter/2_Domainanalyse}
\input{mainmatter/3_Zieldefinition}
\input{mainmatter/4_Technologieevaluation}
\input{mainmatter/5_Konkurrenzanalyse}
\input{mainmatter/6_Produktplanung}
\input{mainmatter/6_Vorgehensmodelle}
\input{mainmatter/7_Durchführungsplanung}
\input{mainmatter/8_Ablaufplanung}
\input{mainmatter/9_Terminplanung}
\input{mainmatter/10_Kostenplanung}
\input{mainmatter/11_Qualitätssicherung}
\input{mainmatter/12_Risikomanagement}
